...
func main() {
	...
	// Sort the games by the logic describe: handType and then highest card in order
...
func (g game) handType() int {
	...
	if count[keys[0]] == 3 && count[keys[1]] == 2 {
		return full_house
	}

	if count[keys[0]] == 3 && count[keys[1]] == 1 {
		return three_kind
	}
	...
	if count[keys[0]] == 2 && count[keys[1]] == 1 {
		return one_pair
	}
